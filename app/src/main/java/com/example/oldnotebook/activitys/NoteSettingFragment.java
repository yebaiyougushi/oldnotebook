		...
		if (mSyncScreenPref == preference) {
			...
			query.findObjects( new FindListener<Note>() {
				...
				public void done(List<Note> list, BmobException e) {
					LogUtils.d(TAG, "query.success:" +list.size());
					Message message = new Message();
					message.what  = MSG_QUERY_ALL_OVER;
					Bundle bundle = new Bundle();
					bundle.putParcelableArrayList("notelist", (ArrayList<? extends Parcelable>) list);
	...
	public class SyncDataTask extends AsyncTask<List<Note>, String, String>{
		...
		protected String doInBackground(List<Note>... params) {
			...
			for (Note note: notesList) {
				...
				String selection = MetaData.NoteColumns.OBJECT_ID + "='" + objectId + "'";
				Cursor cursor = conResolver.query(MetaData.NoteColumns.CONTENT_URI, null, selection, null, null);
...
//						"title='" + title + "'", null);
				...
				if (cursor != null && cursor.getCount() > 0) {
					getActivity().getContentResolver().update(MetaData.NoteColumns.CONTENT_URI, values,
						null, null);
					continue;
				}
