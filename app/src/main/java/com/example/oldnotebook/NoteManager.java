...
public class NoteManager {
	...
	public static Note getNotesFromId(long id, Context context){
		Note noteBook = new Note(id);
		Uri uri = Uri.parse(MetaData.NoteColumns.CONTENT_URI + "/" + id);
		Cursor cur = null;
		try {
			...
			if (cur.moveToFirst()) {
				do {
					noteBook.setObjectId(cur.getString(cur.getColumnIndex(MetaData.NoteColumns.OBJECT_ID)));
					noteBook.setContent(cur.getString(cur.getColumnIndex(MetaData.NoteColumns.CONTENT)));
					noteBook.setGroup(cur.getString(cur.getColumnIndex(MetaData.NoteColumns.GROUP)));
					noteBook.setTime(cur.getLong(cur.getColumnIndex(MetaData.NoteColumns.TIME)));
